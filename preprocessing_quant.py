...
def get_args():
    parser = argparse.ArgumentParser(description="Genera mappe GT usando SAM + CLIP con Palette Ridotta")
...
# --- DATI ADE20K COMPLETI (Riferimento) ---
FULL_ADE20K_CLASSES = [
...
def get_reduced_setup():
    """Restituisce le classi, la palette e gli indici ridotti."""
    final_indices = TARGET_INDICES.copy()
    if 127 not in final_indices and len(FULL_ADE20K_CLASSES) > 127:
        final_indices.append(127) # Animal (Importante per ImageNet)
...
def main(args):
    ...
    for split in ["test", "training"]:
        ...
        out_seg = os.path.join(out_root, "segmentation_quantized", split)
        ...
        for filename in tqdm(image_files):
            try:
                ...
                # --- A. DEPTH ---
                # Eseguiamo anche la depth se non esiste
                ...
                # --- B. SAM + CLIP SEGMENTATION (REDUCED + FIX OUTPUT) ---
                
                # 1. Genera Maschere (SAM)
                outputs_sam = sam_generator(image_pil)
                
                # FIX OUTPUT: Gestione output lista vs dizionario
                if isinstance(outputs_sam, list):
                    # Formato standard: lista di dict con 'mask', 'bbox', ecc.
                    # Ma SAM pipeline a volte ritorna PIL image in 'mask'
                    raw_masks = outputs_sam
                elif isinstance(outputs_sam, dict):
                    # Formato che hai riscontrato: {'masks': [PIL...], 'scores': [...]}
                    # Qui mancano le bbox, dobbiamo calcolarle
                    # Trasformiamo in una lista di dicts compatibile
                    raw_masks = []
                ...
                for m_item in raw_masks:
                    ...
                    if isinstance(m_item, dict):
                        ...
                        bbox = m_item.get('bbox', None) # Potrebbe non esserci
                    else:
                        # Fallback strano
                        continue

                    # Converti in bool array
                    mask_arr = np.array(mask_img) > 0
                    area = np.sum(mask_arr)
                    
                    # Filtra maschere piccolissime
                    if area < 100: continue

                    # Calcola BBox se manca
                    if bbox is None:
                        y_indices, x_indices = np.where(mask_arr)
                        if len(y_indices) == 0 or len(x_indices) == 0: continue
                ...
                for m_data in masks_data:
                    ...
                    # --- FIX: Filtra ritagli troppo piccoli ---
                    # Se l'oggetto è più piccolo di 10x10 pixel, CLIP fallisce o dà warning.
                    # Saltiamo questi "puntini" di rumore.
                    ...
                    with torch.no_grad():
                        ...
                        # Similarity against REDUCED features
