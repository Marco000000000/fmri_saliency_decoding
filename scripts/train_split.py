...
def main():
    ...
    print(f"🧠 Caricamento Training S{args.subject} | Modo: {args.mask_mode.upper()}...")
    ...
    for img_id in tqdm(valid_ids):
        ...
        if img_id_int in id_to_img_filename and img_id_int in id_to_mask_filename:
            ...
            if os.path.exists(img_path) and os.path.exists(mask_path):
                ...
                final_img = apply_smart_mask(img, mask_flat, bbox, args.mask_mode)
                    
                inputs = clip_processor(images=final_img, return_tensors="pt").to(device)
                with torch.no_grad(): clip_embeddings[img_id_int] = clip_model(**inputs).image_embeds.cpu()
    ...
    for ep in range(args.epochs):
        ...
        aligner.eval()
        with torch.no_grad(): val_loss = criterion(aligner(X_val), Y_val)

        print(f"Ep {ep+1:02d} | Train: {train_loss.item():.4f} | Val: {val_loss.item():.4f}", end="")
        if val_loss.item() < best_val_loss:
            best_val_loss = val_loss.item()
            save_name = f"S{args.subject}_clip_aligner_{args.mask_mode[:2]}.pth"
