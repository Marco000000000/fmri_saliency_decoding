...
def main():
    ...
    parser.add_argument("--mask_mode", type=str, required=True, choices=["foreground", "background"], 
                        help="Scegli se addestrare sul soggetto (foreground) o sullo sfondo (background)")
    ...
    # Cartella dove si trovano le maschere .npy pre-calcolate
    masks_dir = f"{base_dir}/data/precomputed_binary_masks"
    
    # Cartella di salvataggio differenziata
    save_dir = f"{base_dir}/trained_fmri_decoders/shen_clip_sparse_{args.mask_mode}"
    os.makedirs(save_dir, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # 1. Caricamento Dati
    print(f"🧠 Caricamento fMRI S{args.subject} | Modo: {args.mask_mode.upper()}...")
    df_tsv = pd.read_csv(tsv_path, sep='\t', header=None)
    
    id_to_img_filename = {int(row[3]): (str(row[0]) + ".JPEG" if not str(row[0]).endswith('.JPEG') else str(row[0])) for _, row in df_tsv.iterrows()}
    id_to_mask_filename = {int(row[3]): (str(row[0]) + ".npy") for _, row in df_tsv.iterrows()}
    ...
    # 2. Estrazione CLIP con Mascheramento Dinamico
    print(f"🎨 Estrazione CLIP ({args.mask_mode.upper()})...")
    ...
    for img_id in tqdm(valid_ids):
        ...
        if img_id_int in id_to_img_filename and img_id_int in id_to_mask_filename:
            img_path = os.path.join(img_dir, id_to_img_filename[img_id_int])
            mask_path = os.path.join(masks_dir, id_to_mask_filename[img_id_int])
            ...
            if os.path.exists(img_path) and os.path.exists(mask_path):
                ...
                # 2. Carica Maschera (piatta 4096 -> 64x64 -> resize alla dimensione dell'immagine)
                mask_flat = np.load(mask_path)
                mask_64 = mask_flat.reshape(64, 64)
                mask_pil = Image.fromarray((mask_64 * 255).astype(np.uint8)).resize(img.size, Image.NEAREST)
                mask_bool = np.array(mask_pil) > 127 # Matrice Booleana
                
                # 3. Applica il mascheramento (sfondo nero)
                masked_img_arr = img_arr.copy()
                if args.mask_mode == "foreground":
                    masked_img_arr[~mask_bool] = 0 # Annerisce lo sfondo
                elif args.mask_mode == "background":
                    masked_img_arr[mask_bool] = 0  # Annerisce il soggetto
                    
                masked_img_pil = Image.fromarray(masked_img_arr)
                
                # 4. Estrai CLIP dell'immagine ritagliata
                inputs = clip_processor(images=masked_img_pil, return_tensors="pt").to(device)
                with torch.no_grad():
    ...
    if missing_masks > 0:
        print(f"⚠️ Attenzione: mancano {missing_masks} immagini o maschere per l'estrazione.")
    ...
    print(f"🔥 Training CLIP {args.mask_mode.upper()} Aligner...")
    ...
    for ep in range(args.epochs):
        ...
        if val_loss.item() < best_val_loss:
            ...
            # Salva con nome differenziato
            save_name = f"S{args.subject}_clip_aligner_{args.mask_mode[:2]}.pth" # diventerà _fg.pth o _bg.pth
            torch.save(aligner.state_dict(), os.path.join(save_dir, save_name))
            print(" [SAVED *]")
        else:
            print("")
